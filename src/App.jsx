...
function Scenarios({ latestKPI, data, allKPIs, scenarios, setScenarios }) {
  ...
  // Feature 6: Rename preset
  const renamePreset = (key) => {
    if (!editLabel.trim()) return;
    const updated = { ...customPresets, [key]: { ...customPresets[key], label: editLabel.trim() } };
    setCustomPresets(updated);
    localStorage.setItem('pulse_presets', JSON.stringify(updated));
    ...
    <div>
      ...
      <div className="scenario-layout">
        ...
        <div className="slider-panel">
              ...
              <div key={key} style={{ position: 'relative' }}>
                {editingPreset === key ? (
                  <div style={{ display: 'flex', gap: 4 }}>
                    <input className="scenario-edit-input" value={editLabel} onChange={e => setEditLabel(e.target.value)} onKeyDown={e => e.key === 'Enter' && renamePreset(key)} autoFocus />
                    <button className="mini-btn" onClick={() => renamePreset(key)}>✓</button>
                    <button className="mini-btn" onClick={() => setEditingPreset(null)}>✗</button>
                  </div>
                ) : (
                  <button className={`scenario-tab-btn ${activeScenario === key ? 'active' : ''}`} onClick={() => selectPreset(key)}
                    onDoubleClick={() => { if (key !== 'custom') { setEditingPreset(key); setEditLabel(p.label); } }}>
                    {p.label}
                  </button>
                )}
              </div>
          ...
          {/* Feature 6: Save/Reset preset buttons */}
          {activeScenario !== 'custom' && (
            <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
              <button className="mini-btn" onClick={() => saveCurrentAsPreset(activeScenario)} title="Aktuelle Slider-Werte in diesem Preset speichern">💾 Preset speichern</button>
              <button className="mini-btn" onClick={resetPresets} title="Alle Presets auf Standardwerte zurücksetzen">↺ Standard</button>
            </div>
          )}
