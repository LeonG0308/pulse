...
async function renderProfessionalPDF({ reportText, charts, companyName, branche, recipientInfo, latestKPI, data, timePeriod, allKPIs }) {
  ...
  pdf.text(`${fmtMonthFull(firstDatum)} – ${fmtMonthFull(lastDatum)}`, W / 2, 145, { align: 'center' });
  ...
  pdf.text('Automatisch erstellt mit PULSE – KI-gestütztes Controlling für KMU', W / 2, H - 22, { align: 'center' });
  ...
  for (const section of sections) {
    if (section.type === 'section_title') {
      ...
      if (numberedMatch) {
        ...
        pdf.setFontSize(11);
        setColor(C.text);
        const lines = pdf.splitTextToSize(content, TW - 14);
...
// ─── Parse report text into structured elements ───
function parseReportText(text) {
  ...
  for (const line of lines) {
    ...
    if (sectionMatch) {
      elements.push({ type: 'section_title', text: sectionMatch[1].trim() });
      continue;
    }

    // Chart marker
    const chartMatch = trimmed.match(/\[\[CHART:(.+?)\]\]/);
    if (chartMatch) {
      elements.push({ type: 'chart', chartId: chartMatch[1].trim() });
      continue;
    }

    // Regular text (skip empty lines but track them for spacing)
    if (trimmed) {
